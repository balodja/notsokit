...
if sys.version_info.major < 3:
	print("ERROR: notsokit requires Python 3.")
	sys.exit(1)

if "CXX" in os.environ:
	cmakeCompiler = os.environ["CXX"]

if "NOTSOKIT_OVERRIDE_CXX" in os.environ:
	cmakeCompiler = os.environ["NOTSOKIT_OVERRIDE_CXX"]

if "NOTSOKIT_OSX_CROSSBUILD" in os.environ:
	enable_osx_crossbuild = True

if "NOTSOKIT_BUILD_TESTS" in os.environ:
	build_tests = True

if shutil.which("cmake") is None:
	print("ERROR: notsokit compilation requires cmake.")
	sys.exit(1)

ninja_available = shutil.which("ninja") is not None
if not (ninja_available or shutil.which("make")):
	print("ERROR: notsokit compilation requires Make or Ninja.")
	sys.exit(1)
try:
	from setuptools import setup # to ensure setuptools is installed
except ImportError:
	print("ERROR: Setuptools is required to install notsokit python module.\nInstall via pip3 install setuptools.")
	sys.exit(1)
...
if options.jobs is not None:
	jobs = options.jobs
if "NOTSOKIT_PARALLEL_JOBS" in os.environ:
	jobs = int(os.environ["NOTSOKIT_PARALLEL_JOBS"])
else:
...
################################################
# functions for cythonizing and building notsokit
################################################

def buildNotsokit(install_prefix, externalCore=False, externalTlx=None, withTests=False, rpath=None):
	try:
	...
	comp_cmd.append("-DNOTSOKIT_FLATINSTALL=ON")
	...
	comp_cmd.append("-DNOTSOKIT_PYTHON="+get_paths()['include']) # provide python.h files
	comp_cmd.append("-DNOTSOKIT_PYTHON_EXECUTABLE="+sys.executable) # provide cmake with Python interpreter
	comp_cmd.append("-DNOTSOKIT_PYTHON_SOABI="+os_soabi) # provide lib env specification
	comp_cmd.append("-DNOTSOKIT_PYTHON_VERSION="+sysconfig.get_python_version())
	comp_cmd.append("-DNOTSOKIT_NUMPY="+np.get_include()) # provide numpy.h files
	if externalCore:
		if sys.platform == "win32":
			...
			exit(1)
		comp_cmd.append("-DNOTSOKIT_BUILD_CORE=OFF")
	if build_tests:
		comp_cmd.append("-DNOTSOKIT_BUILD_TESTS=ON")
	if sys.platform == "win32":
		comp_cmd.append("-DNOTSOKIT_STATIC=ON") # Windows only supports static core builds
		comp_cmd.append("-DNOTSOKIT_BUILDING_STATELIB=ON") # Adds dllexport
		comp_cmd.append("-DNOTSOKIT_PYTHON_VERSION="+sysconfig.get_python_version())
	if enable_osx_crossbuild:
		comp_cmd.append("-DCMAKE_OSX_ARCHITECTURES='arm64'")
	if externalTlx:
		comp_cmd.append("-DNOTSOKIT_EXT_TLX="+externalTlx)
	if ninja_available:
		comp_cmd.append("-GNinja")
	comp_cmd.append(os.getcwd()) #call CMakeLists.txt from notsokit root
	if rpath:
		comp_cmd.append("-DNOTSOKIT_PYTHON_RPATH="+rpath)
	# Run cmake
	print("initializing notsokit compilation with: '{0}'".format(" ".join(comp_cmd)), flush=True)
...
# Our build_ext command compiles notsokit using cmake and installs it into a directory
...
class build_ext(Command):
	...
	user_options = [
		...
		('notsokit-external-core', None,
			"use external notsokit core library"),
		('external-tlx=', None,
	...
	def initialize_options(self):
		...
		self.notsokit_external_core = False
	...
	def get_source_files(self):
		sources = [ ]
		for subdir, dirs, files in os.walk('notsokit'):
	...
	def run(self):
		...
		# However, we know that we only want to build a single extension, i.e. notsokit.
		prefix = self.build_lib
		if self.inplace:
			# The --inplace implementation is less sophisticated than in distutils,
			# but it should be sufficient for notsokit.
			prefix = self.distribution.src_root or os.getcwd()
		buildNotsokit(prefix, externalCore=self.notsokit_external_core, externalTlx=self.external_tlx, rpath=self.rpath)
...
setup(
	...
	package_data		= {'notsokit.profiling': ['html/*','latex/*','description/*']},
	keywords			= version.keywords,
	platforms			= version.platforms,
	classifiers			= version.classifiers,
	cmdclass			= {'build_ext': build_ext},
	ext_modules			= cythonize(["notsokit/*pyx"], compiler_directives=compiler_directives),
...
try:
	...
	print("WARNING: _tkinter is necessary for notsokit.profiling module if not used via jupyter.\nInstall tkinter if needed")
